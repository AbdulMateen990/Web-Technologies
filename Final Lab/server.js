...
//Need to login to Add Products

app.get('/add-product', (req, res) => {
  let isAuthenticated = req.isAuthenticated(); // This depends on your authentication setup
  res.render('add-product', { isAuthenticated: isAuthenticated });
});
app.post('/add-product', (req, res) => {
  ...
  newProduct.save()
      .then(() => res.redirect('/featured-products'))
      .catch(err => res.status(400).send('Error saving product: ' + err.message));
});
...
app.get('/featured-products', (req, res) => {
  ...
  Product.find({ isFeatured: true })
      .then(featuredProducts => {
          res.render('featured-products', { featuredProducts, isAuthenticated });
      })
      .catch(err => res.status(400).send('Error fetching featured products: ' + err.message));
});

app.post('/add-to-session/:id', (req, res) => {
  const productId = req.params.id;
  if (!req.session.productIds) {
      req.session.productIds = [];
  }
  req.session.productIds.push(productId);
...
app.get('/product-details/:id', (req, res) => {
  ...
  Product.findById(objectId)
      .then(product => {
          ...
          res.render('product-details', { product, isAuthenticated });
      })
      .catch(err => res.status(400).send('Error fetching product details: ' + err.message));
});
